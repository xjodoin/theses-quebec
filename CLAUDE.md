...
- **DB is in Git LFS**. Don't commit `data/theses.db` from a stale clone; `git lfs pull` first. Pages deploys on push to `main` when `data/theses.db`, `scripts/build.mjs`, or `web/static.html` change (`.github/workflows/pages.yml`).
